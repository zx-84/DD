...
const defaultPlayers = [
  { name: "Guerrier", hp: 14, maxHp: 14 },
  { name: "Magicien", hp: 8, maxHp: 8 },
  { name: "Voleur", hp: 10, maxHp: 10 },
  { name: "Clerc", hp: 12, maxHp: 12 }
];
...
function scoreOracle(question, entry) {
  const words = normalizeText(question).split(/\W+/).filter(Boolean);
  return words.reduce((score, word) => {
    const hitPrompt = entry.prompts.some((prompt) => normalizeText(prompt).includes(word));
    const hitTag = entry.tags.some((tag) => normalizeText(tag).includes(word));
    const hitTitle = normalizeText(entry.title).includes(word);
    return score + (hitPrompt ? 2 : 0) + (hitTag ? 1 : 0) + (hitTitle ? 1 : 0);
  }, 0);
}

function getOracleReply(question) {
  let bestEntry = oracleEntries[0];
  let bestScore = 0;

  oracleEntries.forEach((entry) => {
    const score = scoreOracle(question, entry);
    if (score > bestScore) {
      bestScore = score;
      bestEntry = entry;
    }
  ...
  if (bestScore === 0) {
    return {
      ...
      answer: "L’oracle local n’a pas trouve de correspondance nette. Essayez avec un nom de lieu, de PNJ, de monstre, ou avec des mots comme repos, puzzle, Cragmaw, Glasstaff ou Nezznar.",
      tags: ["aide", "reformulation"],
      category: "all"
    };
  }
